...
func hash_chain(pedersen_ptr : HashBuiltin*, data_ptr : felt*, length : felt) -> (
    ...
    if length == 2:
        let (pedersen_ptr, result) = pedersen_hash(
            pedersen_ptr=pedersen_ptr, x=[data_ptr], y=[data_ptr + 1])
        return (pedersen_ptr=pedersen_ptr, result=result)
    else:
        let (pedersen_ptr, result) = hash_chain(pedersen_ptr=pedersen_ptr, data_ptr=data_ptr, length=length-1)
        let (pedersen_ptr, result) = pedersen_hash(
            pedersen_ptr=pedersen_ptr, x=result, y=[data_ptr + length - 1])
        return (pedersen_ptr=pedersen_ptr, result=result)
    end
end
